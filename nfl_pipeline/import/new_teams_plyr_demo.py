...
find out where players changed teams and calculate points gained or lost for new/old teams
"""
...
if __name__ == '__main__':
    ...
    df = df[(df['position'].isin(['QB','RB','WR','TE']))*(df['game_type']=='REG')][['season','team','week','player_name','draft_number','age','years_exp']]
    df.rename(columns={'player_name':'player'}, inplace=True)


    df = df.sort_values(by=['season','week']).reset_index(drop=True)
    df['next_team'] = 0
    df['games_played'] = df['years_exp'] * 15
    df.drop(columns=['years_exp'],inplace=True)
    ...
    df_nt_grouped.fillna(0, inplace=True)

    # send results to csv for use in other functions
    df_nt_grouped.to_csv('import_files/new_team_pts.csv', index=False)
    df.to_csv('import_files/player_age_draft_team.csv', index=False)
