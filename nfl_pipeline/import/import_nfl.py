...
def load_combined_data(stat_type, years, weekly_data, output=False):
    ...
    # calculate fantasy points and merge next weeks fantasy points
    df_merge['ffpts'] = add_ff_points(df_merge, stat_type)
    df_merge['ffpts_target'] = pd.NA
    
    player_list = df_merge['player'].unique()
    player_quant = len(player_list)
    current_player = 0
    for player in player_list:
        current_player +=1
        points_list = df_merge[df_merge['player']==player]['ffpts'].to_list()
        next_points_list = points_list[1:]
        next_points_list.append(np.nan)
        df_merge.loc[df_merge['player']==player,'ffpts_target'] = next_points_list
    ...
    file_path = os.path.join('import_files', filename)
...
def new_team_age_draft(df):
    ...
    """
    df_new_data = pd.read_csv('import_files/player_age_draft_team.csv')[['season','week','player','draft_number','age','games_played', 'draft_pos_decay']]
    df_new_team_points = pd.read_csv('import_files/new_team_pts.csv')[['season','week','team','fpts_gained','fpts_lost']]
...
def coach_data_input(df):
    ...
    """
    coach_df = pd.read_csv('import_files/coach_data.csv')
